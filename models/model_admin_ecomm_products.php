...
//            $this->select_child($all_cat, $children, $children_counter);
    ...
    function add_new_product($data = '', $file = "")
    {
        ...
        $pro_count = $this->filter($data["pro_count"]);
        ...
        if (!is_numeric($price)) {
            ...
            $param = [$product_name, $category, $introduction, $summary, $price, $seo, $status, $pro_count];
            if (!empty($product_name) and !empty($category) and !empty($introduction) and !empty($summary) and !empty($status)) {
                $result = $this->Do_Query("insert into ecomm_products(`name`,`category`,`introduction`,`summary`,`price`,`seo`,`status`,`pro_count`)VALUES (?,?,?,?,?,?,?,?)", $param);
                if ($result == true) {
...
//            $result_main = $this->Do_Select("select * from ecomm_product_gallery where main='ACTIVE' and product_id=?", [$producy_id]);
    ...
    function update_product($data = '', $product_id = "")
    {
        ...
        if (!is_numeric($price)) {
            ...
            if (!empty($product_name) and !empty($category) and !empty($introduction) and !empty($summary) and !empty($status)) {
                $param = [$product_name, $category, $introduction, $summary, $price, $seo, $status, $pro_count, $product_id];
                $result = $this->Do_Query("update   ecomm_products set `name`=?,`category`=?,`introduction`=?,`summary`=?,`price`=?,`seo`=?,`status`=?,`pro_count`=? where `id`=?", $param);
                if ($result == true) {
                    unset($_SESSION["product_info"]);
                    $_SESSION["update_product"] = "success";
                    header("location:" . SITE_URL . "admin_ecomm_products");
                } else {
                    $_SESSION["update_product"] = "danger";
                    header("location:" . SITE_URL . "admin_ecomm_products/edit_product/" . $product_id);
                }
            } else {
                $_SESSION["update_product"] = "empty";
                header("location:" . SITE_URL . "admin_ecomm_products/edit_product/" . $product_id);
            }
        }
    }
