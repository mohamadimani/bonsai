...
//  product
    ...
    function get_product_message($product_id = "")
    {
        $results_new = "";
        $product_id = $this->filter($product_id);
        $results = $this->DO_Select("select *  from ecomm_product_message LEFT JOIN `institute_users`  on ecomm_product_message.`user_id`=`institute_users`.`id`  where  ecomm_product_message.product_id=? and ecomm_product_message.`status`='ACTIVE' ORDER by ecomm_product_message.`id` desc   ", [$product_id]);
    ...
    function user_info()
    {
        if (isset($_SESSION["user_id"]) and !empty(trim($_SESSION["user_id"]))) {
            $user_id = $this->filter(base64_decode($_SESSION["user_id"]));
            $result = $this->Do_Select("select * from `institute_users`  where `id`=? ", [$user_id], 1);
        } else {
            $result = [];
        }
        return $result;
    ...
    function checkout($factor_id = "")
    {
        ...
        $_SESSION["verify_callback"] = 'ecomm/pay_verify';
...
//                        $this->send_sms($user_info["mobile"], $text);
                        ...
                        $_SESSION["pay_error"] = "مشکل در اتصال به درگاه پرداخت";
                        header('location:' . SITE_URL . 'ecomm/basket');
                    }
                } else {
                    header('location:' . SITE_URL . 'ecomm/basket');
                ...
                $result = $this->zarinpalRequest($factor_info["amount"], "imt_shop_" . $factor_info["factor_number"], "info@bamboweb.ir", $user_info["mobile"]);
...
فاکتور شماره $factor_number هنوز پرداخت نشده";
    ...
    function get_more_products($product_id = "")
    {
        $product_id = $this->filter($product_id);
        $category = $this->DO_Select("select category   from ecomm_products   where  id=?  ", [$product_id], 1);
        $cat_group = $this->DO_Select("select cat_group   from ecomm_product_category   where  id=?  ", [$category['category']], 1);
        $cat_ids = $this->DO_Select("select id   from ecomm_product_category   where  cat_group=?   ", [$cat_group['cat_group']]);
        $all_ids = [];
        foreach ($cat_ids as $id) {
            $all_ids[] = $id['id'];
        }
        $cat_ids = implode(',', $all_ids);
        $products = $this->DO_Select("select ecomm_products.* , ecomm_product_category.title from ecomm_products LEFT JOIN ecomm_product_category ON ecomm_products.`category`=ecomm_product_category.`id` where ecomm_products.`status`='ACTIVE' and ecomm_products.`category` IN ($cat_ids) and ecomm_products.id!=? ", [$product_id]);
        return $products;
    }
