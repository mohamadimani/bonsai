...
//    set_pages seen log
    function seen_pages($page_names = "index")
    {
        ...
        $is_ip = $this->Do_Select("select * from `page_views_log` WHERE  `view_date` >= $today and `user_ip`=? and `page_name`='index'  ", [$ip], 1);
        if (!empty($is_ip["id"])) {
...
//            'from' => '5000125475',
    ...
    function get_user_info($param = '', $userid = '')
    {
        if (empty($userid)) {
        ...
        if (empty($param)) {
            ...
            return $this->Do_Select("select $param from `institute_users` where `id`=?", [$user_id], 1);
        }
    }
}
