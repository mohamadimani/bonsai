...
//   ===   require view  ===
    ...
    {
        $this->model("headers");
        $user_info = $this->model->get_institute_user_info();
        $site_options = $this->model->get_options();
...
//   ===   require view  ===
    function teacher_view($url = '', $data = [], $header = '', $footer = '')
    {
        $this->model("headers");
        $user_info = $this->model->get_institute_teacher_info();
        $site_options = $this->model->get_options();

        if (empty($header)) {
            $header_url = 'views/teacher/header.php';
            if (file_exists($header_url)) {
                unset($_SESSION['errors']['controller_errors'][04]);
                require($header_url);
        ...
        if (empty($footer)) {
            $footer_url = 'views/teacher/footer.php';
            if (file_exists($footer_url)) {
                require($footer_url);
                unset($_SESSION['errors']['controller_errors'][05]);
            } else {
...
//        $ecomm_category = $this->model->get_ecomm_category();
        ...
        $site_views = $this->model->site_views();
...
//   ===   require view  ===
    function feed_view($url = '', $data = [], $header = '', $footer = '')
    {

        if (empty($header)) {
            $header_url = 'views/feed/header.php';
            if (file_exists($header_url)) {
                require($header_url);
                unset($_SESSION['errors']['controller_errors'][2]);
            } else {
        ...
        if (empty($footer)) {
            $footer_url = 'views/feed/footer.php';
            if (file_exists($footer_url)) {
                require($footer_url);
